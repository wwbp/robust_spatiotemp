...
with connection:
  with connection.cursor(cursors.SSCursor) as cursor:
    ...
    type1_counties = type_mapping.loc[type_mapping['type_num'] == 1, 'cnty'].tolist() # Exurbs
    type2_counties = type_mapping.loc[type_mapping['type_num'] == 2, 'cnty'].tolist() # Graying America
    type3_counties = type_mapping.loc[type_mapping['type_num'] == 3, 'cnty'].tolist() # African American South
    type4_counties = type_mapping.loc[type_mapping['type_num'] == 4, 'cnty'].tolist() # Evangelical Hubs
    type5_counties = type_mapping.loc[type_mapping['type_num'] == 5, 'cnty'].tolist() # Working Class Country
    type6_counties = type_mapping.loc[type_mapping['type_num'] == 6, 'cnty'].tolist() # Military Posts
    type7_counties = type_mapping.loc[type_mapping['type_num'] == 7, 'cnty'].tolist() # Urban Suburbs
    type8_counties = type_mapping.loc[type_mapping['type_num'] == 8, 'cnty'].tolist() # Hispanic Centers
    type9_counties = type_mapping.loc[type_mapping['type_num'] == 9, 'cnty'].tolist() # Native American Lands
    type10_counties = type_mapping.loc[type_mapping['type_num'] == 10, 'cnty'].tolist() # Rural Middle America
    type11_counties = type_mapping.loc[type_mapping['type_num'] == 11, 'cnty'].tolist() # College Towns
    type12_counties = type_mapping.loc[type_mapping['type_num'] == 12, 'cnty'].tolist() # LDS Enclaves
    type13_counties = type_mapping.loc[type_mapping['type_num'] == 13, 'cnty'].tolist() # Aging Farmlands
    type14_counties = type_mapping.loc[type_mapping['type_num'] == 14, 'cnty'].tolist() # Big Cities
    type15_counties = type_mapping.loc[type_mapping['type_num'] == 15, 'cnty'].tolist() # Middle Suburbs
    ...
    fig.set_size_inches(18, 8)
    ax2 = ax.twinx()
    sql = "SELECT week, EST_MSA, gen_health, gad2_sum, phq2_sum FROM household_pulse.pulse where est_msa is not null;"
    household_pulse = pd.read_sql(sql, connection)
    household_pulse['EST_MSA'] = household_pulse['EST_MSA'].astype('str')
    sql = "SELECT a.msa as EST_MSA, a.cnty FROM county_data.superCrossWalk_FINAL_FINAL a LEFT OUTER JOIN county_data.superCrossWalk_FINAL_FINAL b ON a.msa = b.msa AND a.total_pop10 < b.total_pop10 WHERE b.msa IS NULL;"
    household_pulse_msa_mapping = pd.read_sql(sql, connection)
    household_pulse_msa_mapping['EST_MSA'] = household_pulse_msa_mapping['EST_MSA'].astype('str')
    household_pulse_week_mapping = pd.read_csv("./household_pulse_week_mapping.csv") # Add custom yearweeks

    print(household_pulse.head(5))
    print(household_pulse_msa_mapping.head(5))
    print(household_pulse_week_mapping.head(5))

    household_pulse = household_pulse.merge(household_pulse_week_mapping, on='week')
    #household_pulse = household_pulse.merge(household_pulse_msa_mapping, on='EST_MSA')

    # group by week
    household_pulse = household_pulse.groupby(by=['yearweek']).mean().reset_index()
    ...
    anx_line = ax2.plot(x, household_pulse['gad2_sum'], 'b-', label='Worry and Anxiety (0 is best)')
    dep_line = ax2.plot(x, household_pulse['phq2_sum'], 'r-', label='Disinterest and Depression (0 is best)')
    gen_line = ax2.plot(x, household_pulse['gen_health'], 'g-', label='General Health (5 is best)')
    #plot_events()
    ...
    household_cols = ['gen_health', 'gad2_sum', 'phq2_sum']
